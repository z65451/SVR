...
def getMaps(image, attn):
	rows, columns, _ = image.shape
	# energy_map = getEnergy(image)
	# energy_map = cv2.imread("attn.jpg")
	# energy_map = cv2.cvtColor(energy_map, cv2.COLOR_BGR2GRAY)

	s=attn + np.random.normal(5, 15.9, attn.shape)
	ss =255* (s - s.min()) / (s.max() - s.min())


	energy_map = cv2.resize(ss, (image.shape[1], image.shape[0]))
...
def carve(image, attn):

	rows, columns, _ = image.shape
	cMap, goback = getMaps(image, attn)
...
def cropByColumn(image, display_seams, attn, generate = 0, lsit = None, scale_c = 0.5, fromRow = 0):
