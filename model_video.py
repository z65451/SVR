...
class Model(nn.Module):
    ...
    def forward(self, x):
        ...
        #######################################################################
        # Mid part
        # mask_pred_mid = torch.zeros(5,112,224)
        # x_input_mid = torch.zeros(5,3,112,224)

        # for kk in range(mask_pred.shape[0]):
        #     mask_pred_mid[kk] = fn.resize(mask_pred[kk].view(1,224,224), size=[112,224])
        #     x_input_mid[kk] = resize = fn.resize(x_input[kk], size=[112,224])

        # x = mask_pred_mid
        ###########################################################
        # Decoder part (inverse)
...
class MyEnsemble(nn.Module):
    def __init__(self, modelA, modelB):
        ...
        bs = 1
        imgsz = [640,640]
        self.device1 = select_device("")
        

    def forward(self, x1_l_1, x1_r_1, disp_l, disp_r, tmp_img_l_o, tmp_img_r_o):

        x1_l = torch.zeros(5, 3, 224, x_ratio)
        x1_r = torch.zeros(5, 3, 224, x_ratio)
        attn_l = torch.zeros(5, 224, 2*224)
        attn_r = torch.zeros(5, 224, 2*224)



        attn_l=attn_l.to("cuda:0")
        attn_r=attn_r.to("cuda:0")
        disp_l = disp_l - disp_l.min()
        disp_r = disp_r - disp_r.min()


        cv2.imwrite("disp_l.jpg", ((disp_l[2].permute(1,2,0).cpu().numpy())/disp_l.cpu().numpy().max()) *255)
        cv2.imwrite("disp_r.jpg", ((disp_r[2].permute(1,2,0).cpu().numpy())/disp_r.cpu().numpy().max()) *255)
        
        for kk in range(x1_l_1.shape[0]):
            x1_l[kk] = fn.resize(x1_l_1[kk], size=[x1_l_1.shape[2], x_ratio])
            x1_r[kk] = fn.resize(x1_r_1[kk], size=[x1_r_1.shape[2], x_ratio])
        seen, windows, dt = 0, [], (Profile(), Profile(), Profile())
        model = DetectMultiBackend('yolov5/yolov5s.pt', device=self.device1,dnn=False, fp16=False)       
        # model.warmup(imgsz=(1 if pt or self.model.triton else bs, 3, *imgsz))  # warmup

        with dt[1]:
            pred = model(tmp_img_l_o[0].to("cuda:0"), False, False)
            predr = model(tmp_img_r_o[0].to("cuda:0"), False, False)
        with dt[2]:
            pred = non_max_suppression(pred, 0.25, 0.45, None, False, max_det=1000)
            predr = non_max_suppression(predr, 0.25, 0.45, None, False, max_det=1000)
        
        for i, det in enumerate(pred):

            if len(det):
                # Rescale boxes from img_size to im0 size
                det[:, :4] = scale_boxes(tmp_img_l_o[0].shape[2:] , det[:, :4], (375,1242, 3)).round()
                
            for *xyxy, conf, cls in reversed(det):
                if cls == 0 or cls == 1 or cls == 2 or cls == 3 or cls == 4 or cls == 7:
                    xyxy[1] = xyxy[1]*(224/375)
                    xyxy[3] = xyxy[3]*(224/375)
                    xyxy[0] = xyxy[0]*(448/1242)
                    xyxy[2] = xyxy[2]*(448/1242)
                    attn_l[2, int(xyxy[1]):int(xyxy[3]), int(xyxy[0]):int(xyxy[2])] += disp_l[2,0, int(xyxy[1]):int(xyxy[3]), int(xyxy[0]):int(xyxy[2])] 
                    # attn_l += disp_l[:,0]
        ...
        for i, detr in enumerate(predr):
    
            if len(detr):
                # Rescale boxes from img_size to im0 size
                detr[:, :4] = scale_boxes(tmp_img_r_o[0].shape[2:] , detr[:, :4], (375,1242, 3)).round()
                
            for *xyxyr, conf, clsr in reversed(detr):
                if clsr == 0 or clsr == 1 or clsr == 2 or clsr == 3 or clsr == 4 or clsr == 7:
                    xyxyr[1] = xyxyr[1]*(224/375)
                    xyxyr[3] = xyxyr[3]*(224/375)
                    xyxyr[0] = xyxyr[0]*(448/1242)
                    xyxyr[2] = xyxyr[2]*(448/1242)
                    attn_r[2, int(xyxyr[1]):int(xyxyr[3]), int(xyxyr[0]):int(xyxyr[2])] += disp_r[2,0, int(xyxyr[1]):int(xyxyr[3]), int(xyxyr[0]):int(xyxyr[2])] 
                    # attn_r += disp_r[2,0]
        ...
        max_det=1000,
        # x1_l_, attn_l_ = self.modelA(x1_l)
        # x1_r_, attn_r_ = self.modelA(x1_r)


        # # x1_l_2 = torch.zeros(5, 3, 224, 2*224)
        # # x1_r_2 = torch.zeros(5, 3, 224, 2*224)
        
        # attn_l= fn.resize(attn_l_, size=[224, 448])
        # attn_r= fn.resize(attn_r_, size=[224, 448])
        
        cv2.imwrite("hhl.jpg", attn_l[2].cpu().numpy()*255)
        cv2.imwrite("hhr.jpg", attn_r[2].cpu().numpy()*255)

        x3, resized, x2, tt, i_non_zero_start, i_non_zero_end, x3_r, resized_r, x2_r, tt_r, i_non_zero_start_r, i_non_zero_end_r, img_l_out, img_r_out, M_right_to_left, M_left_to_right, V_left, V_right = self.modelB(x1_l_1, attn_l, x1_r_1, attn_r, disp_l, disp_r)
        return x3 , attn_l, x2,tt, i_non_zero_start, i_non_zero_end, x3_r , attn_r, x2_r,tt_r, i_non_zero_start_r, i_non_zero_end_r, img_l_out, img_r_out, M_right_to_left, M_left_to_right, V_left, V_right